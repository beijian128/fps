...
**新增 `tests/ui_smoke.gd`（真实集群 + 真正的主场景）**：登录页注册 → 进大厅（断言**没有**
自动匹配）→ 拉到个人档案（≥1 级）与商城状态（1000 金币）→ 顶栏显示用户名 → 商城页挂上
≥4 张商品卡 → 点开始匹配（对等客户端同队列）→ 配对成功进对局、HUD 可见、大厅隐藏。
结果：`ui_smoke: OK`。
...
| 测试 | 结果 |
|---|---|
| `login_smoke` | `OK (user=smoke_144679 rotated=true)` |
| `ws_smoke` | 两个客户端同局（槽位 0/1），4 秒 79 个唯一 step |
| `rejoin_smoke` | `REJOIN OK`（同一 match_id / 同一槽位 / 重连后 full 帧） |
| `profile_smoke` | `OK`（`logic.logic.profile` 端到端） |

**13 个无头回归 + 5 个冒烟全绿**（见 AGENTS.md §6 的清单）。

**未覆盖（留给人工）**：窗口拉伸与中文字形的肉眼检查；以及「打满一局 → 结算层」——它需要
两名真人对枪到 10 杀，`ui_smoke` 只覆盖到「进局」。**本轮实测是在 1280×720 默认窗口下跑的**，
拉伸检查与结算层观感请在手工游玩时确认。
