extends SceneTree
## 开发回归测试：验证断线时 _on_connection(false) 会把刚体渲染实体、金币节点、
## 本地世界（WorldStore）与插值状态全部清空（否则旧刚体以冻结姿势残留到重连后的
## full 帧才被协调掉，视觉上是整场重建的闪跳）。
...
func _run() -> void:
	...
	# 模拟连接中残留的状态：一个渲染刚体节点 + 一枚金币节点 + 一帧插值状态 + 本地世界。
